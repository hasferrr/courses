...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname championship-bracket-improved-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; winners...and losers.
; 
; "Below is a function" that "takes" a bracket and a team 
; 
; and "produces":
; - the "name of the team" that knocked the given team out of the tournament,
; - or "false" if there's no such team in the bracket (either because 
...
(define (ouster t br)
  (cond [(false? br) false]
        [else
         (if (string=? t (bracket-team-lost br))
             (bracket-team-won br)
             (local [(define check-knock (ouster t (bracket-br-won br)))]
               (if (not (false? check-knock))
                   check-knock
                   (ouster t (bracket-br-lost br))))
             )]
        ))
