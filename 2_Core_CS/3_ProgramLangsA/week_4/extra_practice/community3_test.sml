...
val test2d: bool = do_until (fn (x, acc) => (x - 1, x * acc)) (fn (x, _) => x <> 0) (6, 1) = (0, 720)
val test2z: bool = do_until (fn x => x div 2) (fn x => x mod 2 <> 1) 2 = 1
;
val test03: bool = factorial1 0 = 1
...
val test4z: bool = fixed_point (fn x => x div 2) 10 = 0
;
(* ('a -> 'b) -> ('a * 'a) -> ('b * 'b) *)
...
val test5z: bool = map2 String.size ("a", "bbb") = (1, 3)
;
fun f n = [n, 2 * n, 3 * n]
fun g n = [n * n, 2 * n * n]
...
;
val test7a: bool = myfoldr (fn (a,b) => a^b) "" ["q","w","e"] = foldr (fn (a,b) => a^b) "" ["q","w","e"]
val test7b: bool = myfoldr (fn (a,b) => a^b) "" ["q","w","e"] <> foldl (fn (a,b) => a^b) "" ["q","w","e"]
;
...
val test11c: bool = mymap String.size ["a", "bb", "ccc"] = [1,2,3]
;
...
val test13z: bool = myfoldl (fn (a,b) => a^b) "" ["q","w","e"] = foldl (fn (a,b) => a^b) "" ["q","w","e"]
;
...
fun treeadder (x, y) = x + y
fun treemult (x, y) = x * y
;
...
val test14z: bool = tree_map (fn x => x*x) toptree = node {value= 1, left=midtree4, right=midtree9}
;
